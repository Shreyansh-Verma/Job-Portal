...
class acceptedemployee extends Component {
    
    constructor(props) {
        super(props);
        this.state = {users: JSON.parse(localStorage.getItem("recruitview")),users1:[],users2:'',sortedUsers: [], sortName:true,flag:false,filtusers:[],check:['IIT','BITs','NIT'],filttemp:[],namtemp:[],ustuse:[]};
    ...
    componentDidMount() {
        Promise.all([
            ...
            axios.get('http://localhost:4000/user/jobregister1')
                 ...
                 console.log("json=",JSON.parse(localStorage.getItem("viewemployee")))
                 var abc=JSON.parse(localStorage.getItem("viewemployee"))
                 console.log("abc=",abc)
                 for(var i=0;i<this.state.users2.length;i++)
                 {
                     if(this.state.users2[i].email===abc)
                     {
                         this.state.ustuse.push(this.state.users2[i])
                       //  ustuse=this.state.users2[i]
                     }
                 }
                 console.log("ustuse=",this.state.ustuse)
                 for(var i=0;i<this.state.ustuse.length;i++)
                 {
                     for(var j=0;j<this.state.ustuse[i].appleuser.length;j++)
                     {
                        
                            if(this.state.ustuse[i].appleuser[j].status==="Accepted")
                            this.state.filtusers.push(this.state.ustuse[i].appleuser[j])
              ...
              /*   for (var i=0;i<ustuse.appleuser.length;i++)
                 {
                     ...
                     for(var j=0;j<this.state.users1.length;j++)
                     {
                        ...
                        {
                            if(ustuse.appleuser[i].status==="Accepted")
                    ...
                    <Grid item xs={12} md={9} lg={9}>
                        <Paper>
                            <Table size="small">
                                <TableHead>
                                    <TableRow>
                                            <TableCell> <Button onClick={this.sortChange}>{this.renderIcon()}</Button>Date of Join</TableCell>
                                            <TableCell><Button onClick={this.sortChangename}>{this.renderIcon1()}</Button>Name</TableCell>
                                            <TableCell>Job Type</TableCell>
                                            <TableCell><Button onClick={this.sortChangename}>{this.renderIcon1()}</Button>Job Title</TableCell>
